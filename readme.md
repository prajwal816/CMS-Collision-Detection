...
![SHAP feature-importance summary — Phase 4](assets/phase4_shap_summary.png)
...
- [Assets and Links](#assets-and-links)
...
| Asset | What it contains | Link / Location |
|---|---|---|
| CERN Open Data (primary) | CMS Run2016G DoubleMuon NanoAOD dataset record (real collision data), includes DOI, run range, and metadata | [opendata.cern.ch/record/30522](https://opendata.cern.ch/record/30522) |
| Kaggle ROOT subset | Local ROOT files used for fast EDA + conversion in Kaggle | [kaggle.com/…/doublemuon2016g-rootfiles](https://www.kaggle.com/datasets/katakuricharlotte/doublemuon2016g-rootfiles) |
| Kaggle Parquet dataset | Parquet feature table + certification JSON used for modeling | [kaggle.com/…/parquet-triggeremu](https://www.kaggle.com/datasets/katakuricharlotte/parquet-triggeremu) |
| Phase 4 artifacts | Multi-label models, metrics CSVs, thresholds CSV, plots, config | `/kaggle/working/phase4_artifacts/` (generated by Phase 4 notebook) |
...
|---|---|---|
| Phase 1 (EDA) | Understand NanoAOD structure; validate physics sanity (dimuon mass, feature distributions, HLT inventory) | EDA notebook + exported mass-spectrum plot for README |
| Phase 2 (Build) | Apply certified run/lumi filtering; create dimuon-level feature table; write Parquet shards | `parquet_dimuon/*.parquet`, `validated_runs_muons_2016.txt` |
| Phase 3 (Baseline) | Leakage-safe modeling with GroupKFold by event; basic explainability; initial drift checks | Baseline model metrics + feature importance/SHAP + stability plots |
| Phase 4 (Robust) | Event-level aggregation; multi-label training; calibration + thresholds; drift monitoring; saved artifacts | `metrics_per_label.csv`, `event_metrics_oof.csv`, `thresholds.csv`, `models/*.joblib`, `plots/*.png`, `config.json` |
...
![Event-level PR curve](assets/phase4_HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_DZ_pr.png)

### Drift: mean score vs run bin
![Drift: mean score vs run bin](assets/phase4_HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_DZ_drift_score.png)

### Physics sanity check: Z peak
![Physics sanity check: Z peak](assets/phase1_dimuon_mass_Z.png)
